...
export async function promoteDriverHandler(
  fastify: FastifyInstance,
  request: FastifyRequest,
  reply: FastifyReply,
) {
...
export async function demoteDriverHandler(
  fastify: FastifyInstance,
  request: FastifyRequest,
  reply: FastifyReply,
) {
  ...
  try {
    ...
    if (target.role === "user") {
      return reply.code(200).send({
        message: "User is already a user (not a driver)",
        user: {
          id: target.id,
          username: target.username,
          role: "user",
    ...
    request.log.error({ error }, "Failed to demote user to user");
    return reply.code(500).send({ error: "Internal server error" });
  } finally {
    client.release();
  }
